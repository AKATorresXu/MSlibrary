 ...
 %>
	...
	<div class="container">
		<div class="heading text-center">
			<h2><font face="KaiTi_GB2312">热&nbsp;门&nbsp;图&nbsp;书&nbsp;推&nbsp;荐</font>
          ...
          <div style="text-align: center;">
				<form action="<%=basePath%>BookServlet" method="get"
					...
					<input type="text" name="key" class="btn btn-lg"style="background:#fff;border-color: #d43f3a;color:#303030;" /> <input
						type="hidden" name="path" value="select" /> <input
						type="submit" value="搜索" class="btn btn-danger btn-lg" />&nbsp;&nbsp;&nbsp;<a style="color:red;text-decoration:underline;" href="https://www.baidu.com">百度一下</a>
		...
		<div class="row flat">
			...
			<c:forEach items="${hotBook}" var="hotBook">
				<div class="col-lg-3 col-md-3 col-xs-6">
					<ul class="plan plan1">
						<li class="plan-name">《${hotBook[0].bookname }》</li>
						<li class="plan-price"><strong>${hotBook[0].author }</strong>
							/ 作者</li>
						<li><strong><a style="font-size:36px;"
								href="<c:url value='/BookServlet?path=findBook&bookId=${hotBook[0].id }&cate=${hotBook[1] }'/>"><%= ++i %></a></strong>
							/排名</li>
						<li><strong>${hotBook[1]}</strong> /类别</li>
						<li><strong>${hotBook[0].restcount }</strong> /库存</li>
						<li><strong>￥${hotBook[0].bookprice } 元</strong> /书价</li>
						<li><strong>￥${hotBook[0].borrowprice } 元</strong> /借书价</li>
						<li><strong>${hotBook[0].borrowcount }</strong> /借阅次数</li>
						<li class="plan-action">
						<form method="post" action="<%=basePath%>OrderServlet?path=borrow">
													<input type="hidden" value="${hotBook[0].id }" name="bookId">
													<div style="text-align: center;">
													<input id = "sub" type="submit" value="借阅《${hotBook[0].bookname }》" class="btn btn-danger btn-lg"
													<c:if test="${hotBook[0].restcount==0}">disabled="disabled" title="库存数量不足无法借阅" style="background-color: silver;"</c:if>
